...
class Program
{
    ...
    public void main()
    {
        ...
        var debtors_email = debtors.Where(debtor => debtor.Email.EndsWith("@rhyta.com") || debtor.Email.EndsWith("@dayrep.com")).ToList();
        foreach (var debtor in debtors_email)
        {
            Console.WriteLine(debtor.Email);
        }

        //3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
        var debtors_filtered = debtors.Where(debtor => debtor.BirthDay.Year >= 1994 || debtor.BirthDay.Year <= 1986);
        foreach (var debtor in debtors_filtered)
        {
            Console.WriteLine(debtor.BirthDay);
        }

        //4) Borcu 5000 - den cox olmayan borclularic cixartmag
        var debt = debtors.Where(debtor => debtor.Debt < 5000);
        foreach (var debtor in debt)
        {
            Console.WriteLine(debtor.Debt);
        }

        //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
        var debt_name = debtors.Where(debtor => debtor.FullName.Length > 18 && debtor.Phone.Count(c => c == '7') >= 2);
        foreach (var debtor in debt)
        {
        ...
        var winterBornDebtors = debtors.Where(debtor => debtor.BirthDay.Month == 12 || debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2);
        foreach (var debtor in winterBornDebtors)
        {
        ...
        var winterBorn = debtors.Where(debtor => debtor.BirthDay.Month == 12 || debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2);
        foreach (var debtor in winterBorn)
        {
            Console.WriteLine(debtor.FullName);
        }

        //9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
        var debts = debtors.Where(debtor => debtor.Debt > debtors.Average(debtor => debtor.Debt)).ToList();

        debts.Sort((x, y) => x.FullName.CompareTo(y.FullName));
        debts.Sort((x, y) => x.Debt.CompareTo(y.Debt));
        debts.Sort((d1, d2) => { return Convert.ToInt32(d1.Address.Length > d2.Address.Length); });
        ...
        var debtorsWithThreeSameLetters = debtors.Where(debtor =>
        debtor.FullName.ToLower().Distinct().Count() == 3 &&
        debtor.FullName.ToLower().GroupBy(c => c).Any(g => g.Count() >= 3));

        var sortedDebtors = debtorsWithThreeSameLetters.OrderBy(debtor => debtor.FullName);
        ...
        var date = debts.GroupBy(debtor => debtor.BirthDay.Year).OrderByDescending(g => g.Count()).First().Key;
        Console.WriteLine(date);

        //14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
        var maxDebtors = debtors.OrderByDescending(debtor => debtor.Debt).Take(5).ToList();

        //15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
        var debtorsDebt = debtors.Where(debtor => debtor.Debt > 0);
        decimal totalDebt = debtorsDebt.Sum(debtor => debtor.Debt);
        Console.WriteLine("Umumi Borc: " + totalDebt);

        //16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
        var debtors_war = debtors.Where(debtor => debtor.BirthDay.Year >= 1941 && debtor.BirthDay.Year <= 1945) ;
        foreach (var debtor in debtors_war)
        {
            Console.WriteLine(debtor.FullName);
        }

        //18)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
        var debtorsWithRepeatedDigits = debtors.Where(debtor => debtor.Phone.Distinct().Count() != debtor.Phone.Length);
        foreach (var debtor in debtorsWithRepeatedDigits)
        {
            Console.WriteLine(debtor.Debt + " " + debtor.Phone);
        }
        ...
        var debtorsWithTargetWord = debtors.Where(debtor =>
        {
            ...
            {
                if (fullName.Count(ch => ch == c) < targetWord.Count(ch => ch == c))
                {
                    return false;
                }
            }
